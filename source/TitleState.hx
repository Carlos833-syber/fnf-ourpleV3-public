...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		var ourp:FlxSprite =
			new FlxSprite().loadGraphic(Paths.image('ourp'));

		ourp.scale.set(2.5, 2.5);
		ourp.updateHitbox();

		ourp.setPosition(
			FlxG.width - ourp.width - 100,
			(FlxG.height - ourp.height) / 2
		);
		...
		logoBl.frames =
			Paths.getSparrowAtlas('logobump');

		logoBl.animation.addByPrefix(
			'bump',
			'logobump idle',
			12,
			false
		...
		statics.frames =
			Paths.getSparrowAtlas('VHS');

		statics.animation.addByPrefix(
			's',
			'VHS',
			12
		);
